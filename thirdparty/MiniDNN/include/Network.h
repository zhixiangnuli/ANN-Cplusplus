...
private:
    ...
    MetaInfo get_meta_info() const
    {
        ...
        map.insert(
            std::make_pair( "OutputLayer", internal::output_id( m_output->output_type() ) ) );
        return map;
    }
...
public:
    ...
    ///
    /// \param opt        An object that inherits from the Optimizer class, indicating the
    /// optimization algorithm to use. \param x          The predictors. Each column is an
    /// observation. \param y          The response variable. Each column is an observation. \param
    /// batch_size Mini-batch size. \param epoch      Number of epochs of training. \param seed Set
    /// the random seed of the %RNG if `seed > 0`, otherwise
    ///                   use the current random state.
    ///
              ...
              std::vector<std::vector<MiniDNN::Scalar>>& paras = nullptr, double test_loss = 0.0 )
    {
        ...
        typedef Eigen::Matrix<typename PlainObjectX::Scalar, PlainObjectX::RowsAtCompileTime,
                              PlainObjectX::ColsAtCompileTime>
            XType;
        typedef Eigen::Matrix<typename PlainObjectY::Scalar, PlainObjectY::RowsAtCompileTime,
                              PlainObjectY::ColsAtCompileTime>
                  YType;
        const int nlayer = num_layers();
    ...
    void export_net( const std::string& folder, const std::string& filename ) const
    {
        ...
        if ( !created ) throw std::runtime_error( "[class Network]: Folder creation failed" );
    ...
    void read_net( const std::string& folder, const std::string& filename )
    {
        ...
        std::vector<std::vector<Scalar>> params =
            internal::read_parameters( folder, filename, nlayer );
        m_layers.clear();
